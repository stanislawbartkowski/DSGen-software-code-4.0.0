...
-- 
 ...
 from
   (select ss_ticket_number
    ...
    and ({{ ref('household_demographics') }}.hd_dep_count = [DEPCNT] or
         {{ ref('household_demographics') }}.hd_vehicle_count= [VEHCNT])
    and {{ ref('date_dim') }}.d_dow in (6,0)
    and {{ ref('date_dim') }}.d_year in ([YEAR],[YEAR]+1,[YEAR]+2) 
    and {{ ref('store') }}.s_city in ('[CITY_A]','[CITY_B]','[CITY_C]','[CITY_D]','[CITY_E]') 
