...
-- 
 ...
 select c_last_name
   ...
   (select ss_ticket_number
    ...
    and ({{ ref('household_demographics') }}.hd_buy_potential = '[BPONE]' or
         {{ ref('household_demographics') }}.hd_buy_potential = '[BPTWO]')
    and {{ ref('household_demographics') }}.hd_vehicle_count > 0
    and case when {{ ref('household_demographics') }}.hd_vehicle_count > 0 then 
             {{ ref('household_demographics') }}.hd_dep_count/ {{ ref('household_demographics') }}.hd_vehicle_count else null end > 1
    and {{ ref('date_dim') }}.d_year in ([YEAR],[YEAR]+1,[YEAR]+2)
    and {{ ref('store') }}.s_county in ('[COUNTY_A]','[COUNTY_B]','[COUNTY_C]','[COUNTY_D]')
    group by ss_ticket_number,ss_customer_sk) dj,{{ ref('customer') }}
